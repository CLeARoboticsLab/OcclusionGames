...
class JetRacerController:
    def __init__(self):
        ...
        rospy.Subscriber(VEL_TOPIC, TwistStamped, self.twist_callback)

        self.logfile = open("dynamics_csv/circle_dynamics_fernando_2.csv", "w")
        self.writer = csv.writer(self.logfile)
        self.writer.writerow(["Time", "steering", "throttle", "x", "y", "vx", "vy", "velocity", "yaw", "yaw_rate", "Error"])
  ...
  # New callback
    def pose_callback(self, msg):
        ...
        # get velocity as current as possible
        #vel_msg = rospy.wait_for_message(VEL_TOPIC, TwistStamped)
        #vel_msg = vel_msg.twist
        #velocity_normed = math.hypot(vel_msg.linear.x, vel_msg.linear.y)

        self.writer.writerow([now - self.start_time, steering, throttle, x, y, self.last_vx, self.last_vy, math.hypot(self.last_vx, self.last_vy), yaw, self.last_omega, radial_error])
        self.logfile.flush()

    def twist_callback(self, msg):
        self.last_vx = msg.twist.linear.x
        self.last_vy = msg.twist.linear.y
        self.last_omega = msg.twist.angular.z
